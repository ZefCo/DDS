...
output_file = "AutoCorrs_10_Safe.csv"
...
projectpath = paste(root_root, "Coding", "Cpp", "DDS", "AC10_Proj", sep = seperator)
...
for (i in 1:length(rootfolder)) {
  ...
  sf10 <- auto_core(master_frame$SFk10)
  sfx <- rep(1:length(sf10) - 1)
  area10[[i]] <- trapz(unlist(sfx), unlist(sf10))
...
sfn <- measure_count(area10, areamax)


output_data <- data.frame(Temp = t_index, E = e_index, Size = s_index, S10tau = unlist(area10), S10tmax = unlist(areamax), S10n = unlist(sfn))
