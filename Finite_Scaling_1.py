...
scaling_folder = cwd / "ScalingImages_Nu"
...
#             subdata["y"] = parallel**(-(b/n)) * ((numpy.sqrt(parallel*orthongonal)) * subdata["Rad01"])

#             # subdata["tLo"] = subdata["t"] * (orthongonal**(2/3))
#             # subdata = subdata.sort_values("tLp")

#             fig1.add_trace(go.Scatter(x = subdata["x"], y = subdata["y"], name = f"{rxc}", mode = "markers"))
#             fig1.add_trace(go.Scatter(x = subdata["x"], y = subdata["y"], mode = "lines", line = dict(dash = "dash", color  = "black", width = 0.3)))
#             # fig.add_trace(go.Scatter(x = subdata["tLp"], y = subdata["SFk10"], name = f"{rxc}_10"))

#         fig1.update_layout(title = f"B = {b}\tNu = {n}")

#         # fig.show()
#         fig1.write_image(str(scaling_folder_1 / figName1))


for n in nu_p:
    fig = go.Figure()
    name = format(n, ".2f")
    figName = f"N_{name}.png"
    ...
    for rxc in sizes:
        ...
        subdata = subdata[(subdata["x"] <= 3) & (subdata["x"] >= -3)]

        fig.add_trace(go.Scatter(x = subdata["x"], y = subdata["BC01"], name = f"{rxc}", mode = "markers"))
        fig.add_trace(go.Scatter(x = subdata["x"], y = subdata["BC01"], mode = "lines", line = dict(dash = "dash", color  = "black", width = 0.3)))
    
    fig.update_layout(title = f"Nu = {n}")
