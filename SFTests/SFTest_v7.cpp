...
int main() {
    ...
    std::vector<std::vector<int>> spin_lattice;
    ...
    lattice.resize(R);
    spin_lattice.resize(R);
    for (int r = 0; r < R; r++) {
        lattice[r].resize(C);
        spin_lattice[r].resize(C);
    }

    std::cout << "\nInit Vertical Lattice" << std::endl;
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            if (c < C / 2) {lattice[r][c] = 1; spin_lattice[r][c] = 1;}
            else {lattice[r][c] = 0; spin_lattice[r][c] = -1;}
        }
    }
    ...
    std::cout << "FFT with 0, 1" << std::endl;

    std::cout << "K = (0, 0) = " << out2D[0][REAL] << " + " << out2D[0][IMAG] << "i" << std::endl;
    ...
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            in2D[c + (C * r)][REAL] = spin_lattice[r][c];
        }
    }

    fftw_execute(p);

    std::cout << "FFT with +- 1" << std::endl;
    ...
    std::cout << "Finished FFT of R" << std::endl;
    ...
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            if (r < R / 2) {lattice[r][c] = 1; spin_lattice[r][c] = 1;}
            else {lattice[r][c] = 0; spin_lattice[r][c] = -1;}
        }
    }
    ...
    std::cout << std::endl << "Finished FFT of R" << std::endl;
    ...
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            if (lattice[r][c] == 1) {spin_lattice[r][c] = 1;}
            else {spin_lattice[r][c] = -1;}
        }
    }
